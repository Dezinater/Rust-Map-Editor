...
public class WorldExample2 : MonoBehaviour
{
    ...
    float[,] shortMapToFloatArray(TerrainMap<short> terrainMap)
    {
    ...
    float[,] byteMapToFloatArray(int channel, TerrainMap<byte> terrainMap)
    {
        float[,] heights = new float[terrainMap.res, terrainMap.res];
        for (int i = 0; i < heights.GetLength(0); i++)
        {
            for (int j = 0; j < heights.GetLength(1); j++)
            {
                heights[i, j] = BitUtility.Byte2Float(terrainMap[channel, i, j]);
            }
        }
        return heights;
    }
    ...
    private string GetInfo(WorldSerialization blob)
	{
        ...
        foreach(WorldSerialization.PrefabData prefab in blob.world.prefabs)
        {
            Debug.Log(prefab.category + " " + prefab.id);
        }
        ...
        terrain.terrainData.heightmapResolution = heightMap.res;
        terrain.terrainData.size = terrainSize;
        terrain.terrainData.SetHeights(0, 0, shortMapToFloatArray(heightMap));
        ...
        water.terrainData.heightmapResolution = waterMap.res;
        water.terrainData.size = terrainSize;
        water.terrainData.SetHeights(0, 0, shortMapToFloatArray(waterMap));
        ...
        float[,,] splatMapFloat = new float[splatMap.res, splatMap.res, 8];

        
        for(int i = 0; i < splatMapFloat.GetLength(0); i++)
        {
            for (int j = 0; j < splatMapFloat.GetLength(1); j++)
            {
                for (int k = 0; k < 8; k++)
                {
                    splatMapFloat[i, j, k] = BitUtility.Byte2Float(splatMap[k, i, j]);
                }
            }
        }

        terrain.terrainData.alphamapResolution = splatMap.res;
        terrain.terrainData.SetAlphamaps(0, 0, splatMapFloat);
